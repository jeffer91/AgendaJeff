...
(function initGoogleCalendarUiEvents(global) {
  ...
  function isUsefulValue(value) { return value !== null && value !== undefined && String(value).trim().length > 0; }
  ...
  function mergePreferForm(firebaseData, formData) {
    ...
    Object.keys(formSource).forEach(function eachKey(key) {
      if (typeof formSource[key] === "boolean") { result[key] = formSource[key]; return; }
      if (isUsefulValue(formSource[key])) result[key] = formSource[key];
    });
    return result;
  }
  ...
  async function readFirebaseConnectionForUi() {
    ...
    if (!connection.readConnection) return { readResult: null, connectionData: {} };
    const readResult = await connection.readConnection();
    const connectionData = readResult && readResult.data && readResult.data.connection ? readResult.data.connection : {};
  ...
  function findBridge() {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      try { if (candidates[index].AgendaJeffElectron) return candidates[index].AgendaJeffElectron; } catch (error) {}
    }
    return null;
  }

  async function readReturnFromElectron() {
    const bridge = findBridge();
    const auth = googleCalendar.Auth || {};
    const pending = auth.readPendingAuth ? auth.readPendingAuth() : null;
    if (!bridge || typeof bridge.getGoogleCalendarReturn !== "function") return { ok: false, found: false };
    return bridge.getGoogleCalendarReturn(pending && pending.state ? pending.state : "");
  }
  ...
  async function runUiAction(label, action) {
    ...
    try {
      ...
      const result = { ok: false, status: "error", action: "uiAction", source: "ui", file: "modulos/googlecalendar/ui/gc-ui-events.js", message: "Falló una acción de interfaz Google Calendar.", error: { message: error && error.message ? error.message : String(error), file: "modulos/googlecalendar/ui/gc-ui-events.js" }, checkedAt: new Date().toISOString() };
      if (render.renderResult) render.renderResult(result);
      return result;
    } finally {
  ...
  async function handleSave() {
    ...
    return runUiAction("Guardando conexión Google Calendar...", async function saveAction() {
      if (!connection.saveConnection) throw new Error("No está disponible Connection.saveConnection.");
  ...
  async function handleLoad() {
    ...
    return runUiAction("Cargando conexión Google Calendar...", async function loadAction() {
      if (!connection.readConnection) throw new Error("No está disponible Connection.readConnection.");
      const result = await connection.readConnection();
      const loadedConnection = result && result.data ? result.data.connection : null;
      if (loadedConnection && dom.fillConnectionForm) dom.fillConnectionForm(loadedConnection);
      if (render.renderLayerStatus) render.renderLayerStatus();
      return result;
    });
  }
  ...
  async function handleClear() {
    ...
    return runUiAction("Limpiando conexión Google Calendar...", async function clearAction() {
      if (!connection.clearConnection) throw new Error("No está disponible Connection.clearConnection.");
      const result = await connection.clearConnection({ all: true });
      if (dom.clearSensitiveFields) dom.clearSensitiveFields();
      return result;
    });
  }
  ...
  async function handleConnect() {
    ...
    return runUiAction("Preparando autorización Google Calendar...", async function connectAction() {
      if (!auth.startDesktopAuth) throw new Error("No está disponible Auth.startDesktopAuth.");
  ...
  async function handleExchangeCode() {
    ...
    return runUiAction("Procesando autorización Google Calendar...", async function exchangeAction() {
      ...
      const payload = returned && returned[callbackKey] ? returned[callbackKey] : null;
      if (!built.authInput[valueKey] && payload && payload[valueKey]) built.authInput[valueKey] = payload[valueKey];
      if (payload && payload.state) built.authInput.state = payload.state;
      const result = await auth.exchangeAuthorizationCode(built.authInput);
      const bridge = findBridge();
      if (result && result.ok && bridge && typeof bridge.clearGoogleCalendarReturn === "function") await bridge.clearGoogleCalendarReturn();
      return result;
    });
  }
  ...
  async function handleTestFirebase() {
    ...
    return runUiAction("Probando Firebase Google Calendar...", async function testFirebaseAction() {
      if (!firebase.testFirebaseConnection) throw new Error("No está disponible Firebase.testFirebaseConnection.");
  ...
  async function handleTestGoogle() {
    ...
    return runUiAction("Probando Google Calendar...", async function testGoogleAction() {
      if (!connection.testConnection) throw new Error("No está disponible Connection.testConnection.");
  ...
  async function handleCreateTestEvent() {
    ...
    return runUiAction("Creando evento de prueba...", async function createTestEventAction() {
      if (!api.createTestEvent) throw new Error("No está disponible Api.createTestEvent.");
  ...
  async function handleDiagnostic() {
    ...
    return runUiAction("Ejecutando diagnóstico Google Calendar...", async function diagnosticAction() {
      if (!diagnostic.runDiagnosticReport) throw new Error("No está disponible Diagnostic.runDiagnosticReport.");
  ...
  function attachEvents() {
    ...
    const bindings = [[elements.btnSave, handleSave], [elements.btnLoad, handleLoad], [elements.btnClear, handleClear], [elements.btnConnect, handleConnect], [elements.btnExchangeCode, handleExchangeCode], [elements.btnTestFirebase, handleTestFirebase], [elements.btnTestGoogle, handleTestGoogle], [elements.btnCreateTestEvent, handleCreateTestEvent], [elements.btnDiagnostic, handleDiagnostic]];
    bindings.forEach(function bindButton(pair) {
      const button = pair[0];
      const handler = pair[1];
      if (button && !button.dataset.gcBound) { button.addEventListener("click", handler); button.dataset.gcBound = "true"; }
    });
    return { ok: true, bound: bindings.filter(function countBound(pair) { return Boolean(pair[0]); }).length };
  }

  ui.Events = Object.freeze({ mergePreferForm, readFirebaseConnectionForUi, buildAuthInputFromUi, findBridge, readReturnFromElectron, runUiAction, handleSave, handleLoad, handleClear, handleConnect, handleExchangeCode, handleTestFirebase, handleTestGoogle, handleCreateTestEvent, handleDiagnostic, attachEvents });
})(window);
